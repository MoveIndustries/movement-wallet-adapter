...
import { MovementWalletAdapterProvider } from "../src/WalletProvider";
...
vi.mock("@moveindustries/wallet-adapter-core", async () => {
  const actual = await vi.importActual("@moveindustries/wallet-adapter-core");
  return {
    ...actual,
...
describe("MovementWalletAdapterProvider", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
  });
  ...
  describe("initial state", () => {
    it("should provide initial disconnected state", async () => {
      render(
        <MovementWalletAdapterProvider disableTelemetry>
          <TestConsumer />
        </MovementWalletAdapterProvider>
      );
    ...
    it("should set isLoading to false after initialization", async () => {
      render(
        <MovementWalletAdapterProvider disableTelemetry autoConnect={false}>
          <TestConsumer />
        </MovementWalletAdapterProvider>
      );
  ...
  describe("auto-connect", () => {
    it("should not auto-connect when autoConnect is false", async () => {
      render(
        <MovementWalletAdapterProvider autoConnect={false} disableTelemetry>
          <TestConsumer />
        </MovementWalletAdapterProvider>
      );
    ...
    it("should not auto-connect when no wallet name is stored", async () => {
      render(
        <MovementWalletAdapterProvider autoConnect={true} disableTelemetry>
          <TestConsumer />
        </MovementWalletAdapterProvider>
      );
    ...
    it("should auto-connect when autoConnect changes from false to true asynchronously", async () => {
      ...
      localStorage.setItem("MovementWalletName", "TestWallet");
      ...
      function AsyncAutoConnectWrapper() {
        ...
        return (
          <MovementWalletAdapterProvider autoConnect={autoConnect} disableTelemetry>
            <TestConsumer />
          </MovementWalletAdapterProvider>
        );
      }
    ...
    it("should auto-connect when wallet registers late after initial render", async () => {
      ...
      localStorage.setItem("MovementWalletName", "LateWallet");
  ...
  describe("provider props", () => {
    it("should accept optInWallets prop", () => {
      expect(() =>
        render(
          <MovementWalletAdapterProvider optInWallets={["Petra"]} disableTelemetry>
            <div>Test</div>
          </MovementWalletAdapterProvider>
        )
      ).not.toThrow();
